...
static void allocate_bg_inode_table(struct block_group_info *bg)
{
	...
	// flexbg inode tbl block location
	u32 block = bg->flexbg_tbl_block;
...
{
	// MStar Android Patch Begin
	// Calculate the metadata location when flexbg enable
	int flexbg_sz = info.flex_bg_size;
	int flexbg_grp = i / flexbg_sz;
	int flexbg_offs = i % flexbg_sz;
	int is_flexbg_first = (flexbg_offs > 0)? 0:1;
	u32 header_blocks = 0;

	/* the last flex bg is not a full flex group */
	if (is_flexbg_first) {
		header_blocks = (2 + aux_info.inode_table_blocks) * flexbg_sz;
		...
		info.flexbg_free_block = header_blocks;
	} else {
		header_blocks = 0;
		bg->has_superblock = 0;
	}
	...
	if (info.flexbg_free_block > info.blocks_per_group) {
		bg->header_blocks = info.blocks_per_group;
		info.flexbg_free_block -= info.blocks_per_group;
	} else {
		bg->header_blocks = info.flexbg_free_block;
		info.flexbg_free_block = 0;
	}
	bg->first_block = aux_info.first_data_block + i * info.blocks_per_group;
	bg->first_flex_bg = flexbg_grp * info.flex_bg_size;
	bg->first_flexbg_block = aux_info.first_data_block
		  + bg->first_flex_bg * info.blocks_per_group;

	u32 block = bg->first_flexbg_block;

	if (aux_info.bgs[bg->first_flex_bg].has_superblock)
		block += 1 + aux_info.bg_desc_blocks +  info.bg_desc_reserve_blocks;

	bg->flexbg_tbl_block = block + 2 * flexbg_sz
			+ flexbg_offs * aux_info.inode_table_blocks;
	block += flexbg_offs;
	bg->flexbg_block_bitmap = block;
	bg->flexbg_inode_bitmap = block + flexbg_sz;
	sparse_file_add_data(info.sparse_file, bg->block_bitmap, info.block_size,
			bg->flexbg_block_bitmap);

	sparse_file_add_data(info.sparse_file, bg->inode_bitmap, info.block_size,
			bg->flexbg_inode_bitmap);
...
static struct region *ext4_allocate_partial(u32 len)
{
	...
	for (i = 0; i < aux_info.groups; i++) {
                // MStar Android Patch Begin
                // Allocate blocks if the bg has free space
                if (aux_info.bgs[i].free_blocks != 0) {
                // MStar Android Patch End
