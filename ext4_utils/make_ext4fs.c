...
// Maths log2 for group num per flexbg calculate.
...
// MStar Android Patch Begin
// Compute flexbg size
static u32 compute_flex_bg_size()
{
	u32 i, flex_size;

	i = int_log2(aux_info.groups);
	if ((u32)(1 << i) == aux_info.groups) {
		flex_size = aux_info.groups;
	} else if ((u32)(1 << i) < aux_info.groups) {
		flex_size = (1 << (i + 1));
	} else {
		printf("Err calc of flex bg size, use default value:512\n");
		/* 4KB block size, 512 groups equal to 64GB */
		flex_size = 512;
	}
                ...
                const char *mountpoint, struct selabel_handle *sehnd)
{
	...
	return make_ext4fs_internal(fd, NULL, mountpoint, NULL, 0, 1, 0, 0, sehnd, 0, 0, 0);
}
                ...
                const char *mountpoint, struct selabel_handle *sehnd)
{
	...
	status = make_ext4fs_internal(fd, NULL, mountpoint, NULL, 0, 0, 0, 1, sehnd, 0, 0, 0);
	// MStar Android Patch End
	close(fd);
...
int make_ext4fs_internal(int fd, const char *_directory,
                         ...
                         struct selabel_handle *sehnd, int verbose
                         , unsigned char padByte, int enablePad)
// MStar Android Patch End
{
	u32 root_inode_num;
	u16 root_mode;
	...
	// MStar Android Patch Begin
	// Calculate and round_up flexbg group size
	if (info.flex_bg_size == 0) {
		info.flex_bg_size = compute_flex_bg_size();
	} else if (info.flex_bg_size == 1) {
		info.flex_bg_size = 1;
	} else if ((info.flex_bg_size & (info.flex_bg_size - 1)) != 0) {
		info.flex_bg_size = compute_flex_bg_size();
		printf("flex bg size must be a power of 2, calc it:%d\n",
			info.flex_bg_size);
	}

	info.log_groups_per_flex = int_log2(info.flex_bg_size);
	info.flexbg_grps = aux_info.groups / info.flex_bg_size;
	if (aux_info.groups % info.flex_bg_size != 0)
		info.flexbg_grps += 1;

	if (info.flex_bg_size > 1) {
		info.feat_incompat |= EXT4_FEATURE_INCOMPAT_FLEX_BG;
		printf("    Flexbg size: %d\n", info.flex_bg_size);
		printf("    Flexbg groups: %d\n", info.flexbg_grps);
	}
	// MStar Android Patch End
	...
	// MStar Android Patch Begin
	// Add pad option
	write_ext4_image(fd, gzip, sparse, crc, padByte, enablePad);
	// MStar Android Patch End
