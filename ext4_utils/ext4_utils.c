...
// MStar Android Patch Begin
// Add pad option
void write_ext4_image(int fd, int gz, int sparse, int crc, unsigned char padByte, int enablePad)
{
	sparse_file_write(info.sparse_file, fd, gz, sparse, crc, padByte, enablePad);
}
// MStar Android Patch End
...
{
	// MStar Android Patch Begin
	// Add flexbg related member init
	aux_info.max_block = 0;
	aux_info.max_len = 0;
	// MStar Android Patch End
	...
	if(hash_directory_index)
		aux_info.default_i_flags |= EXT4_INDEX_FL;
	// MStar Android Patch End
...
void ext4_fill_in_sb()
{
	...
	// MStar Android Patch Begin
	// Update flexbg info in sb and aux_info
	sb->s_log_groups_per_flex = info.log_groups_per_flex;
	...
	for (i = 0; i < aux_info.groups; i++) {
		u64 group_start_block = aux_info.first_data_block + i *
			info.blocks_per_group;
		//u32 header_size = 0;
		if (ext4_bg_has_super_block(i) && (0 == i)) {
		#if 0
			if (i != 0) {
		...
		aux_info.bg_desc[i].bg_block_bitmap = aux_info.bgs[i].flexbg_block_bitmap;
		aux_info.bg_desc[i].bg_inode_bitmap = aux_info.bgs[i].flexbg_inode_bitmap;
		aux_info.bg_desc[i].bg_inode_table = aux_info.bgs[i].flexbg_tbl_block;
