...
#define MOD_MPOOL_NAME             "/dev/malloc"
...
int pm_map_usage_flags(pm_map_t *map, pm_memusage_t *usage_out,
    ...
    // MStar Android Patch Begin
    // skip external memory map by /dev/malloc (mmap)
    if (!strcmp(map->name, MOD_MPOOL_NAME)) {
        memset(usage_out, 0x00, sizeof(pm_memusage_t));
        return 0;
    }
    // MStar Android Patch End
